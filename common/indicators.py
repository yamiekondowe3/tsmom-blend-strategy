...
def ema(series: pd.Series, period: int) -> pd.Series:
    """Standard EMA. `adjust=False` is the recursive form every charting
    package uses, so values match what a trader sees on the platform."""
...
def macd(close: pd.Series, fast: int = 12, slow: int = 26,
    ...
    The signal line is an EMA OF THE MACD LINE. Worth stating because the
    reference TypeScript this was ported from re-seeds a scalar EMA over the
    whole MACD history on every call, which converges to a different value
    than the running series does; this returns the running series.
    """
